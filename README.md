# Zipeek

![rustc](https://img.shields.io/badge/rustc-1.94.0-blue.svg)
![CI](https://github.com/Jozefpodlecki/zippek/actions/workflows/ci.yml/badge.svg)
![wasm-bindgen](https://img.shields.io/badge/wasm--bindgen-0.2.114-blue.svg)

A statically indexed Chinese lexicon using client-side lookup and precomputed sharding.

### Credits  

- <a href="https://www.flaticon.com/free-icons/asia" title="asia icons">Asia icons created by Freepik - Flaticon</a>